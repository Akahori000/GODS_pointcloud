...
""" Code for embedding poses as a bag-of-words model. The code uses kmeans for learning the words/centroids
 ...
 Written by Jue Wang (intern) and Anoop Cherian"""
...
parser.add_argument('--gt', default='./data/gt/Video_list4.txt', type=str, help='ground truth file where the pose file name: See extract_pose_from_video argument list')
parser.add_argument('--poses_root', default='/homes/cherian/kdata/juewang_share/Japan_Pose/Sample_Level_Anomaly+multi-class/', type=str, help='Where are the extracted poses be saved?')
parser.add_argument('--embed_path', default='./data/embed_anoop/', type=str, help='path to pkl filename to store the embedded pose data. It will be saved as embed_path/data_train<split_num>.pkl and embed_path/data_test<split_num>.pkl')
...
if poses_file == 0:
    print('%s file does not exist! This file is the list of all CD1/normal/20180206143224_000.avi files in this format.' % (poses_file_list))    
...
keypt_index_remove = [10,11,13,14,19,20,21,22,23,24] # pose keypoint ids to remove from using in the learning setup. We remove lower body parts as they are not visible in the videos.
imsize = [480,640] # image size.
num_centroids = 256 # number of centroids.

## pose pre-processing.
if args.verbose: print('pose pre-processing!')
...
for video_name in tqdm(poses_file.readlines()):
    ...
    if video_name.split('/')[-2] == 'normal': # the format of video is assumed to be 'x/normal/video.avi' and x/normal/video.avi/pose.pkl' is where the pose is stored.
        label=1
    else:
        label=0
...
data_tr = data_tr
data_va = data_va
if args.verbose: print('Running KMeans and learning BoW centroids...')
...
if args.verbose: print('Embedding poses using bow model...')
data_train=[]
data_test=[]

seq_len = 120; stride=30; beta=15.0
for pose in data_tr:    
    pose = np.array(pose)
...
if args.verbose: print('saving train and test splits to %s and %s...' % (train_embed_file, test_embed_file))
