...
def compute_kernel(A, B, sigma, same=False):
...
def calculate_accruacy(opt, data, label_gt, sigma):
    label = []
    ...
    # kernel for test set.
    KK_test = compute_kernel(data, X, sigma, same=False)
...
# compute prediction accuracy.
def calculate_AUC(opt, data, label_gt, sigma):
    ftr_roc_auc = 0
    tpr = []
    fpr = []
    cls0 = []
    ...
    if (min(tpr) == 0 and min(fpr) == 0 and max(cls1) == len(label_gt) and max(cls0) == len(label_gt)):
        ...
        plt.grid()
        plt.savefig("/workspace/result/KGODS/sigma" + str(sigma) + "/" + str(k) + "/auc_" + anomaly_object + "_subdim_" + str(k) + "_auc_" + str(round(ftr_roc_auc,3)) + ".png")
...
if __name__ == "__main__":
    ...
    for num in range(5):
        for anomalycls in range(0,7):
            ...
            K = compute_kernel(X, X, sigma, same=True)
            ...
            accu_tr, _, _, _ = calculate_accruacy(Xopt, data_tr, label_tr, sigma)
            
            re = calculate_AUC(Xopt, data_va, label_va, sigma) 
    ...
    df.to_csv("/workspace/result/KGODS/sigma" + str(sigma) + "/" + + str(args.num_subspaces) + "/auc_" + "_subdim_" + str(args.num_subspaces) + ".csv")
    print(result)
    df1 = pd.DataFrame(tr_accuracy)
    df1.to_csv("/workspace/result/KGODS/sigma" + str(sigma) + "/" + + str(args.num_subspaces) + "/train_accuracy_" + "_subdim_" + str(args.num_subspaces) + ".csv")
